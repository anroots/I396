...
public class InvoiceRowGenerator {
	...
	public void generateRowsFor(int amount, Date start, Date end) {
		...
		for (; i < paymentDates.size(); i++) {	
			if (remainingAmount - paymentPart <= 3) break;
			
			int rowAmount = (int) paymentPart;
			
			invoiceRows.add(new InvoiceRow(BigDecimal.valueOf(rowAmount), paymentDates.get(i)));
			
			remainingAmount -= rowAmount;
		}
		
		invoiceRows.add(new InvoiceRow(BigDecimal.valueOf(remainingAmount), paymentDates.get(i)));
