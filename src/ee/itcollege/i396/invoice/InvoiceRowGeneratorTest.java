...
public class InvoiceRowGeneratorTest {
	...
	public void generateRowsThreeAmountGeneratesOneCorrectInvoice() {
		ArgumentCaptor<InvoiceRow> rows = ArgumentCaptor
				.forClass(InvoiceRow.class);

		Date startDate = asDate("2012-01-01");
		Date endDate = asDate("2012-03-01");

		generator.generateRowsFor(3, startDate, endDate);
		verify(invoiceRowDao).save(rows.capture());

		assertTrue(rows.getValue().equals(
				new InvoiceRow(BigDecimal.valueOf(3), startDate)));
	}
	...
	public void generateRowsLargerAmountGeneratesCorrectly() {
		...
		generator.generateRowsFor(10, startDate, endDate);
		...
		expectedRows.add(new InvoiceRow(BigDecimal.valueOf(3),
				asDate("2012-02-15")));
		expectedRows.add(new InvoiceRow(BigDecimal.valueOf(3),
				asDate("2012-03-01")));
		expectedRows.add(new InvoiceRow(BigDecimal.valueOf(4),
				asDate("2012-04-01")));
	...
	@Test
	public void generateRowsPutsCentsToTheLastPayment() {
		ArgumentCaptor<InvoiceRow> rows = ArgumentCaptor
				.forClass(InvoiceRow.class);

		Date startDate = asDate("2012-02-15");
		Date endDate = asDate("2012-03-02");

		// We get 2 payments, both 4.5e. The 0.5 cents are put to the last payment
		generator.generateRowsFor(9, startDate, endDate);
		verify(invoiceRowDao, times(2)).save(rows.capture());
		...
		expectedRows.add(new InvoiceRow(BigDecimal.valueOf(4),
				asDate("2012-02-15")));
		expectedRows.add(new InvoiceRow(BigDecimal.valueOf(5),
				asDate("2012-03-01")));
	...
	public void getPaymentDatesNegativeDateDiffThrows()
			throws IllegalArgumentException {
		generator.getPaymentDates(asDate("2012-01-01"), asDate("2011-01-01"));
	}
	...
	public void getFirstOfNextMonthIsCorrect() {
		...
		assertThat(generator.getFirstOFNextMonth(date),
				is(asDate("2012-02-01")));

		date = asDate("2012-01-01");
		assertThat(generator.getFirstOFNextMonth(date),
				is(asDate("2012-02-01")));

		date = asDate("2012-02-28");
		assertThat(generator.getFirstOFNextMonth(date),
				is(asDate("2012-03-01")));

		date = asDate("2012-12-31");
		assertThat(generator.getFirstOFNextMonth(date),
				is(asDate("2013-01-01")));
	}
